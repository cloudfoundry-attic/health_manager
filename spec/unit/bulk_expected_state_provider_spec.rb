...
describe HealthManager::BulkBasedExpectedStateProvider do
  ...
  describe "each_droplet" do
    ...
    let(:bulk_results){ {} }
    let(:http_response_body1) { encode_json({"bulk_token" => "fake-token", "results" => bulk_results }) }
    let(:http_response_body2) { http_response_body1 }
    let(:http_response_body3) { http_response_body2 }
    ...
    context "when the http request is not successful" do
      context "when the failures are intermittent" do
        ...
        before do
          ...
          http_mock.should_receive(:callback).exactly(2).times do |&block|
            block.call if flags.must_succeed
          end
          http_mock.should_receive(:errback).exactly(2).times do |&block|
